...
func TestPipelineFeedsHostLoadSignals(t *testing.T) {
	...
	if feedback.successes != 2 {
		t.Fatalf("successes = %d, want 2", feedback.successes)
	}
	if len(frontier.outcomes) != 4 || !frontier.outcomes[0] ||
		!frontier.outcomes[1] || frontier.outcomes[2] || frontier.outcomes[3] {
