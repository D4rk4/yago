...
func pdfWithText(t *testing.T, title, content string) []byte {
	...
	pdf.WriteString("3 0 obj\n<< /Type /Page /Resources << /Font << /F1 4 0 R >> >> " +
		"/Contents 2 0 R >>\nendobj\n")
...
func TestPDFLiteralEdges(t *testing.T) {
	for escaped, expected := range map[string]byte{
		"n": '\n', "r": '\r', "t": '\t', "b": '\b', "f": '\f',
	} {
		got, consumed := pdfEscapedBytes([]byte(escaped))
		if len(got) != 1 || got[0] != expected || consumed != 1 {
			t.Fatalf("escape %s = %v/%d", escaped, got, consumed)
		}
	}
	for named, octal := range map[string]string{
		"n": "012", "r": "015", "t": "011", "b": "010", "f": "014",
	} {
		namedBytes, _ := pdfEscapedBytes([]byte(named))
		octalBytes, _ := pdfEscapedBytes([]byte(octal))
		if !bytes.Equal(namedBytes, octalBytes) {
...
func pdfTeXStyle(t *testing.T, content string) []byte {
	...
	pdf.WriteString("%PDF-1.7\n1 0 obj\n<< /Type /Page /Resources " +
		"<< /Font << /F58 6 0 R >> >> /Contents 5 0 R >>\nendobj\n" +
		"5 0 obj\n<< /Filter /FlateDecode /Length 99 >>\nstream\n")
	pdf.Write(z.Bytes())
	pdf.WriteString("\nendstream\nendobj\n" +
		"6 0 obj\n<< /Type /Font /Encoding " +
		"<< /BaseEncoding /WinAnsiEncoding /Differences [13 /f_l] >> >>\nendobj\n%%EOF")
