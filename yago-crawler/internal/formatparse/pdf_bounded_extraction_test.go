...
func TestPDFTextCollectorHandlesOperandEdges(t *testing.T) {
	...
	if string(raw) != "outer (inner)\nA" || consumed != len("(outer (inner)\n\\101)")-1 {
...
func TestPDFBoundedTextDecodesMappedAndUTF16Values(t *testing.T) {
	...
	control := newPDFTextCollector(3)
	pdfWriteDecodedStringBytes(control, []byte{0x1C})
	if control.String() != "" {
		t.Fatalf("bounded control = %q", control.String())
	}
}
