...
func (q *progressDeliveryQueue) deliver(ctx context.Context, delivery progressDelivery) {
	if q.leaseGrants != nil && delivery.report.LeaseID == "" {
		q.settle(delivery, crawllease.ErrLeaseLost)
	...
	if status.Code(err) == codes.FailedPrecondition {
		if q.leaseGrants != nil {
			q.leaseGrants.Revoke(delivery.report.LeaseID)
		}
...
func (q *progressDeliveryQueue) settle(delivery progressDelivery, deliveryErr error) {
	...
	if exists && sequence[0].version == delivery.version {
		current := sequence[0]
		switch {
		case deliveryErr == nil || errors.Is(deliveryErr, crawllease.ErrLeaseLost) ||
			!current.terminal:
