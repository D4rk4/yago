...
func TestOrderStreamReceiveFailureIsLogged(t *testing.T) {
	...
	if !strings.Contains(logged, msgOrderStreamReceiveFailed) ||
		!strings.Contains(logged, "storage failed") {
...
func (f *fakeStreamer) AckOrder(
	...
	if len(in.GetOrderIdentity()) != 0 && len(in.GetConfirmationToken()) == 0 {
...
func (f *fakeStreamer) Heartbeat(
	...
	result := &crawlrpc.WorkerHeartbeatResult{Directives: f.beatDirectives}
	if f.renewActive {
		result.RenewedLeaseIds = append([]string(nil), in.GetActiveLeaseIds()...)
		milliseconds := f.leaseTTL.Milliseconds()
		if milliseconds > 0 {
			result.LeaseTtlMilliseconds = uint64(milliseconds)
		}
	}
...
func (f *fakeStreamer) workerRegistrations() []*crawlrpc.WorkerRegistration {
	f.mu.Lock()
	defer f.mu.Unlock()

	return append([]*crawlrpc.WorkerRegistration(nil), f.registrations...)
}
...
func TestGRPCOrderReceiverPreservesHistoricalPayloadIdentity(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	historicalPayload := []byte(
		`{"Provenance":"aGlzdG9yaWNhbA==","Profile":{"Name":"historical"},"Requests":[]}`,
	)
	client := &fakeStreamer{
		ctx: ctx,
		attempts: []streamAttempt{{results: []recvResult{{msg: &crawlrpc.CrawlOrderMessage{
			OrderJson: historicalPayload,
			LeaseId:   "lease-historical",
		}}}}},
	}

	receiver := NewGRPCOrderReceiver(ctx, client, "worker-historical", nil)
	delivery := awaitOrder(t, receiver)
	want := sha256.Sum256(historicalPayload)
	if string(delivery.OrderIdentity) != string(want[:]) {
		t.Fatalf(
			"order identity = %x, want exact payload identity %x",
			delivery.OrderIdentity,
			want,
		)
	}
	reencoded, err := yagocrawlcontract.MarshalCrawlOrder(delivery.Order)
	if err != nil {
		t.Fatalf("re-encode historical order: %v", err)
	}
	reencodedIdentity := sha256.Sum256(reencoded)
	if reencodedIdentity == want {
...
func TestLeaseConfirmationPrecedesOrderDelivery(t *testing.T) {
	...
	stream := &fakeOrderStream{ctx: ctx, results: []recvResult{
		orderResult(t, "confirmed"),
		{err: io.EOF},
	}}
	...
	select {
	case delivery := <-out:
		if delivery.LeaseID != "lease-confirmed" {
			t.Fatalf("delivered lease = %q", delivery.LeaseID)
		}
	default:
		t.Fatal("confirmed lease was not delivered")
	}
	if !registry.Confirmed("lease-confirmed") {
		t.Fatal("delivered lease grant is not confirmed")
	}
	requests := client.heartbeatRequests()
	if len(requests) != 1 || requests[0].GetWorkerSessionId() != "session" ||
		len(requests[0].GetActiveLeaseIds()) != 1 ||
		requests[0].GetActiveLeaseIds()[0] != "lease-confirmed" {
		t.Fatalf("confirmation heartbeat = %+v", requests)
	}
}

func TestOmittedLeaseConfirmationStopsOrderDelivery(t *testing.T) {
	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()
	registry := crawllease.NewGrantRegistry(ctx, 1)
	...
	if len(out) != 0 {
		t.Fatal("unconfirmed lease was delivered")
	}
	if active := registry.ActiveLeaseIDs(); len(active) != 0 {
		t.Fatalf("unconfirmed active leases = %v", active)
	}
}

func TestSessionReceiverDefersHeartbeatUntilStreamRegistration(t *testing.T) {
	restore := orderHeartbeatInterval
	t.Cleanup(func() { orderHeartbeatInterval = restore })
	orderHeartbeatInterval = 250 * time.Millisecond
	ctx, cancel := context.WithCancel(context.Background())
	registry := crawllease.NewGrantRegistry(ctx, 1)
	streamed := make(chan struct{}, 1)
	handler := &recordingControlHandler{}
	...
	case <-time.After(time.Second):
		t.Fatal("session order stream did not register")
	}
	if client.beatCallCount() != 0 {
		t.Fatal("session receiver sent a pre-registration startup heartbeat")
	}
	registrations := client.workerRegistrations()
	if len(registrations) == 0 || registrations[0].GetWorkerSessionId() != "session" {
		t.Fatalf("worker registrations = %+v", registrations)
	}
	...
	if len(handler.snapshot()) == 0 {
		t.Fatal("idle session did not receive control on bounded periodic heartbeat")
	}
	requests := client.heartbeatRequests()
	if len(requests) == 0 || requests[0].GetWorkerSessionId() != "session" {
		t.Fatalf("periodic heartbeat requests = %+v", requests)
	}
	cancel()
	drainUntilClosed(t, receiver)
}
...
func TestDeliverOrderSendsWhenReadable(t *testing.T) {
	...
	delivery := <-out
	if string(delivery.OrderIdentity) != string(identity) {
		t.Fatalf("delivery identity = %x, want %x", delivery.OrderIdentity, identity)
	}
