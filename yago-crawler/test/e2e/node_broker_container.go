...
func remapNodeBroker(t *testing.T, ctx context.Context, node nodeBroker) nodeBroker {
	t.Helper()
	node.opsURL = mappedBaseURL(t, ctx, node.container, nodeOpsPort)
	node.publicURL = mappedBaseURL(t, ctx, node.container, nodePublicPort)
	node.crawlRPCAddress = mappedAddress(t, ctx, node.container, nodeCrawlRPCPort)
