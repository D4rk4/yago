...
A first container package under a personal GitHub account starts private even when it is linked to a public source repository; repository linkage inherits access permissions, not visibility. GitHub exposes no supported REST or GraphQL visibility mutation. Before a release introduces a new GHCR package name, protect the workflow's `release-container-public-visibility` environment with an owner reviewer. The tag workflow publishes and attests the exact manifest, then the protected public-verification job waits without entering a failed state. The owner makes the package public in Package settings and approves that job; its anonymous tag-and-digest gate must pass before GitHub Release creation. Remove the one-time reviewer rule only after the package is irreversibly public. Registry, network, digest, or identity failures remain immediate failures and are never treated as a visibility wait.
