...
- P0 maintenance: Keep README, FEATURES, crawler documentation, and plans aligned with the implemented document store, search backend abstraction, embedded pure-Go Bleve index, Tavily-compatible agent API, YaCy-compatible search adapters, document-backed snippets, RWI generation, and YaCy P2P compatibility.
...
Issue stewardship: Correspond with reporters in the factual, courteous style of old-school Sun/IBM engineering. Verify the report against the current code, documentation, and supported operating procedure before replying. Distinguish confirmed facts from hypotheses, explain the correct procedure and its consequences when a workaround or operator action exists, and request only the minimum evidence needed to reproduce any remaining defect. After an externally reported defect is fixed and its validation is complete, reply on the original issue with a concise closure note. State the observed symptom, root cause, corrective change, and verification evidence; identify the release that carries the fix when one exists; thank the reporter for the actionable feedback. Keep every note self-contained and free of promotional language. Do not claim completion before the evidence is available.

Release issue review: Before every release candidate is feature-closed, inspect all open GitHub issues carrying the exact `bug` or `enhancement` label. Assess whether each can be corrected safely within the candidate's existing scope, implement and validate the feasible corrections before tagging, and record an explicit factual disposition for every reviewed issue. Other labels do not enter this mandatory release-scope review unless the operator asks for them. Never use issue review to move or reuse a tag, bypass the release gate, or silently expand the release after tagging.
...
Public operational data: Treat raw operator or user prompts, search queries, submitted URLs, request and response bodies, production corpus excerpts, account identifiers, credentials, and privately supplied reporter payloads as non-public operational input. Never reproduce them verbatim in release notes, ADRs, `FEATURES.md`, documentation, commit messages, public issue replies, or test fixtures. Preserve the engineering evidence with neutral synthetic values, roles, aggregate measurements, and bounded ranges. Exact public protocol fixtures and material already published by an upstream reporter are allowed only when required for correctness. Before every tag or public reply, search the candidate text for copied operational input, correct every finding, and record the anonymization audit.
...
GitHub documents both a private default for first personal-account packages and repository-visibility inheritance for packages created by a `GITHUB_TOKEN` workflow, so observed anonymous registry access is authoritative. Before a release introduces a new GHCR package name, protect the workflow's `release-container-public-visibility` environment with an owner reviewer. The tag workflow publishes and attests the exact manifest, then the protected public-verification job waits without entering a failed state. Test the exact package tag without registry credentials. If it remains private, the owner makes it Public in Package settings; otherwise no visibility change is required. Approve the waiting job only after anonymous access is observed, require its independent tag-and-digest gate before GitHub Release creation, and remove the one-time reviewer rule only after that gate succeeds. GitHub exposes no supported REST or GraphQL visibility mutation. Registry, network, digest, or identity failures remain immediate failures and are never treated as a visibility wait.

A semantic-version tag remains immutable even when its release workflow fails before publication. Never move, delete, or reuse that tag and never create release assets manually to complete it. Correct the failure on `main`, close the full release gate again, document the unpublished failed tag in the next release memo, and use the next semantic version. A failed tag without a GitHub Release or final published package/container set is release-attempt history, not the previous published release.
...
Research uncertainty rule: Whenever the best implementation, architecture, or operational choice is uncertain, research the alternatives on the internet and arXiv.org before deciding. Prefer primary evidence, record material findings in `CONTINUITY.md`, and identify any conclusion that remains uncertain.
...
Testing: Code lands with tests, written in the same change as the code they exercise — never deferred to a follow-up. Every guard lands with the test that proves it refuses. A test that only exercises the accepting path proves the code runs, not that the rule holds: it passes identically if the guard is deleted. So for each validation, authorization, admission, quality, or budget check, assert both directions — the input it admits and the input it turns away — and for each repair, migration, or defaulting path, assert that already-correct input comes back unchanged. Where a failure and a legitimate empty answer share one response shape, pin the negative case explicitly: prove the honest empty answer is not reported as a failure. Statement coverage does not substitute for this; a branch can be executed by a test that never asserts what the branch decided. Pure documentation/configuration changes need lightweight validation only. For code changes, run focused tests first when useful, then `make verify`. Record exact commands and results in `CONTINUITY.md` and the final response. If a test cannot be added or run, state the concrete reason and residual risk.
...
Gate: `make verify` is the mandatory code gate. It includes checksum-pinned standalone gosec, an exact NilAway finding ratchet, reusable-module API compatibility against the exact previous published tag and commit, symbol-level govulncheck, explicit race-instrumented tests, a recorded deterministic test shuffle, and bounded active fuzz smoke across the applicable production modules. A code change is not done until it is green. Feature closure adds the required Dockerized Semgrep, Dockerized Trivy, container, sanity, smoke, commit, and push checks.
