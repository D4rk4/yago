...
Release procedure: A release ships exactly what is on `main`, so land and feature-close every change first (tests, `make verify`, Dockerized scans, `FEATURES.md` and docs, commit, push). Then cut the release in strict order. First, tag the released `main` commit with a semantic-version `vX.Y.Z` tag and push the tag — that tag push is the only release trigger. The single CI workflow `.github/workflows/release.yml` fires on `v*` tags, runs `make verify`, then builds the `yago-node` and `yagocrawler` binaries for linux `amd64` and `arm64` with the version injected via ldflags from the tag (`${GITHUB_REF_NAME#v}`), and publishes the `.deb`, `.rpm`, and tarball artifacts on the GitHub Release through `deploy/debian/build-deb.sh` and `deploy/rpm/build-rpm.sh`. Never hand-build or upload release artifacts locally; the tag is the source of truth and the tag and the injected build version stay identical. Before finalizing the GitHub Release, write and verify detailed human-authored release notes from the actual released diff and validation evidence. The notes cover operator-visible features and fixes, behavior and compatibility changes, upgrade and deployment considerations, security and reliability impact, validation, and known limitations; an autogenerated commit list may supplement but never replace them. Deploy a node only from the published artifact for its architecture, never from a local build. On the bare-metal production node the upgrade order is: fetch the release `.deb`, back up `/opt/yago/data`, stop `yago-node` and `yagocrawler`, install with `dpkg -i`, then manually start the services (the package postinst enables but does not start them), and verify both came up. Production package installs and service restarts are operator-authorized actions, never automatic.
