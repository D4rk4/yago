...
// for EVERY family the fetch gate admits the document under all-on toggles,
// the parser produces an indexable page, and the extraction carries the
// expected text (and title where the format has one). The same sample must be
// REJECTED at the gate once its family is toggled off, proving the gate and
// the parser consult the same registry. HTML is admitted by the gate but
// declined by Parse — it belongs to the caller's own web-text extractor.
func TestEveryFormatPassesGateAndExtracts(t *testing.T) {
	allOnToggles := Toggles{
	...
	for name, sample := range formatSamples() {
		...
		if name == "html" {
			assertHTMLDeclined(t, sample, body, allOnToggles)

			continue
		}
		assertFamilyExtracts(t, name, sample, body, allOnToggles)
	}
}

// assertHTMLDeclined pins the HTML hand-off: the gate admits web text, IsHTML
// claims it, and Parse declines it to the caller's own extractor.
func assertHTMLDeclined(t *testing.T, sample familySample, body []byte, on Toggles) {
	t.Helper()
	if !IsHTML(sample.url, sample.mime) {
		t.Fatal("html: IsHTML must route the sample to the caller's extractor")
	}
	if _, parsed := Parse(sample.url, sample.mime, body, on); parsed {
		t.Fatal("html: Parse must decline HTML to the caller's extractor")
	}
}

// assertFamilyExtracts pins one family sample end to end: it parses under
// all-on toggles, carries the expected text and title, and the gate rejects
// it once its family toggle is off.
func assertFamilyExtracts(
	t *testing.T,
	name string,
	sample familySample,
	body []byte,
	...
	t.Helper()
	page, parsed := Parse(sample.url, sample.mime, body, on)
	if !parsed {
	...
	if Accepts(sample.url, sample.mime, Toggles{}) {
