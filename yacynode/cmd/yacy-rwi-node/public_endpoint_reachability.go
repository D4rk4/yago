...
func (p publicEndpointSelfTest) queryURL() url.URL {
	target := *p.base
	target.Path = joinPublicSelfTestPath(p.base.Path, yacyproto.PathQuery)
	target.RawQuery = yacyproto.QueryRequest{
...
func (p publicEndpointSelfTest) confirm(ctx context.Context, resp *http.Response) bool {
	...
	msg, _ := yacymodel.ParseMessage(string(body))
	parsed, err := yacyproto.ParseQueryResponse(msg)
	if err != nil {
		slog.DebugContext(ctx, "public endpoint self-test failed", slog.Any("error", err))

		return false
	}
	if parsed.Response == yacyproto.QueryResponseRejected {
