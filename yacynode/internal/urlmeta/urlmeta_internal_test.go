...
func TestIntakeMapsUpdateCapacityAndErrors(t *testing.T) {
	...
	receipt, err := module.Receiver.Receive(t.Context(), []yacymodel.URIMetadataRow{urlRow(t, "a")})
	if err != nil {
		t.Fatal(err)
	}
	if !receipt.Busy {
...
func TestIntakeDiscardsBadRowsAndReportsStoreFailures(t *testing.T) {
	...
	row := urlRow(t, "a")
	receipt, err = module.Receiver.Receive(t.Context(), []yacymodel.URIMetadataRow{row})
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntakeReturnsReadError(t *testing.T) {
	...
	if _, err := module.Receiver.Receive(t.Context(), []yacymodel.URIMetadataRow{row}); err == nil {
...
func TestDirectoryReturnsReadErrors(t *testing.T) {
	...
	if _, err := module.Directory.RowsByHash(t.Context(), []yacymodel.Hash{hash}); err == nil {
		t.Fatal("expected RowsByHash read error")
	}
	if _, err := module.Directory.MissingURLs(t.Context(), []yacymodel.Hash{hash}); err == nil {
...
func TestDirectoryHandlesMissingRowsAndViewErrors(t *testing.T) {
	...
	rows, err := module.Directory.RowsByHash(
		t.Context(),
		[]yacymodel.Hash{yacymodel.WordHash("absent")},
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPurgeReportsDeleteErrorAndAbsentRows(t *testing.T) {
	...
	if err := storage.Update(t.Context(), func(tx *vault.Txn) error {
		_, err := module.Evictor.Purge(t.Context(), tx, []yacymodel.Hash{hash})
		if err != nil {
...
func (failingURLReceiver) Receive(context.Context, []yacymodel.URIMetadataRow) (Receipt, error) {
	return Receipt{}, errors.New("receive failed")
}

type busyURLReceiver struct{}

func (busyURLReceiver) Receive(context.Context, []yacymodel.URIMetadataRow) (Receipt, error) {
	return Receipt{
		Busy:     true,
		Double:   1,
		ErrorURL: []yacymodel.Hash{yacymodel.WordHash("bad")},
	}, nil
}
...
func TestTransferURLReportsReceiveErrorAndBusy(t *testing.T) {
	_, err := (transferURLEndpoint{identity: localIdentity(), intake: failingURLReceiver{}}).Serve(
		t.Context(),
		yacyproto.TransferURLRequest{NetworkName: "freeworld", YouAre: localIdentity().Hash},
	)
	if err == nil {
...
func (okURLReceiver) Receive(context.Context, []yacymodel.URIMetadataRow) (Receipt, error) {
	return Receipt{}, nil
}
...
func TestMountTransferURLServesRoute(t *testing.T) {
	...
	httpReq := httptest.NewRequestWithContext(
		t.Context(),
		http.MethodPost,
		yacyproto.PathTransferURL+"?"+req.Form().Encode(),
		nil,
	)
