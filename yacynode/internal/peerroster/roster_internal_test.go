...
func internalHashFor(base string) yacymodel.Hash {
	if len(base) >= yacymodel.HashLength {
		return yacymodel.Hash(base[:yacymodel.HashLength])
	}
	return yacymodel.Hash(base + internalHashFiller[len(base):])
}

func internalSeed(t testing.TB, hash, ip string) yacymodel.Seed {
	t.Helper()
...
func corruptPeerRecord(t *testing.T, r *roster, engine *scriptedEngine, peer yacymodel.Hash) {
	t.Helper()
