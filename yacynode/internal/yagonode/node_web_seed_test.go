...
type fakeCrawlQueue struct {
	keys   []string
	orders []yacycrawlcontract.CrawlOrder
}
...
func TestWebCrawlSeederPublishesUnknownURLs(t *testing.T) {
	...
	seeder := newWebCrawlSeeder(queue, docs, yacymodel.Hash("node"), webFallbackConfig{
		SeedDepth:    1,
		SeedMaxPages: 20,
	})
	...
	if order.Requests[0].Mode != yacycrawlcontract.CrawlRequestModeURL {
