...
type OutboundRetryDelayFraction func(peer yacymodel.Hash, failures int) float64
...
type OutboundRetryPolicy struct {
	...
	peerStates map[yacymodel.Hash]OutboundRetryState
}
...
func NewOutboundRetryPolicy(config OutboundRetryConfig) *OutboundRetryPolicy {
	return &OutboundRetryPolicy{
		...
		peerStates: make(map[yacymodel.Hash]OutboundRetryState),
	}
}
...
func (p *OutboundRetryPolicy) Ready(peer yacymodel.Hash, at time.Time) bool {
...
func (p *OutboundRetryPolicy) PeerState(peer yacymodel.Hash) (OutboundRetryState, bool) {
	state, ok := p.peerStates[peer]

	return state, ok
}

func (p *OutboundRetryPolicy) failureDelay(peer yacymodel.Hash, failures int) time.Duration {
...
func defaultOutboundRetryDelayFraction(peer yacymodel.Hash, failures int) float64 {
	value := uint64(0)
