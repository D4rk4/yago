...
func TestJSONEndpointRejectsBadRequests(t *testing.T) {
	...
	for _, query := range cases {
		...
		req := httptest.NewRequestWithContext(
			t.Context(),
			http.MethodGet,
			yacyproto.PathYaCySearchJSON+query,
			nil,
		)
...
func TestSearchRequestFromValuesMergesOperatorsAndParams(t *testing.T) {
	values := url.Values{
		...
		yacyproto.FieldLanguage:         {"en"},
		yacyproto.FieldSiteHost:         {"example.org"},
		yacyproto.FieldMaximumRecords:   {"3"},
		yacyproto.FieldStartRecord:      {"2"},
		yacyproto.FieldResource:         {"global"},
		yacyproto.FieldContentDom:       {"all"},
		yacyproto.FieldFileType:         {"html"},
		yacyproto.FieldURLMaskFilter:    {".*"},
		yacyproto.FieldPreferMaskFilter: {"docs"},
		yacyproto.FieldVerify:           {"cacheonly"},
		yacyproto.FieldNavigation:       {"none"},
	}
