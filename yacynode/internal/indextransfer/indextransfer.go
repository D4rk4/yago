...
var (
	...
	parseTransferMessage = yacymodel.ParseMessage
)
...
type transferPost[T any] struct {
	...
	path   string
	form   url.Values
	parse  func(yacymodel.Message) (T, error)
}
...
func (w HTTPPeerWriter) TransferRWI(
	...
	if len(postings) == 0 {
		return yacyproto.TransferRWIResponse{Result: yacyproto.ResultOK}, nil
	}
	...
	return postTransfer(
		transferPost[yacyproto.TransferRWIResponse]{
			ctx:    ctx,
			client: w.client,
			peer:   peer,
			path:   yacyproto.PathTransferRWI,
			form:   req.Form(),
			parse:  yacyproto.ParseTransferRWIResponse,
		},
	)
}
...
func (w HTTPPeerWriter) TransferURL(
	...
	return postTransfer(
		transferPost[yacyproto.TransferURLResponse]{
			ctx:    ctx,
			client: w.client,
			peer:   peer,
			path:   yacyproto.PathTransferURL,
			form:   req.Form(),
			parse:  yacyproto.ParseTransferURLResponse,
		},
	)
}
...
func readTransferMessage(body io.Reader) (yacymodel.Message, error) {
...
func wordCount(postings []yacymodel.RWIPosting) int {
	seen := make(map[yacymodel.Hash]struct{}, len(postings))
