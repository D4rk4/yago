...
func TestExpanderPassesURLRequestsThrough(t *testing.T) {
	...
	if len(got) != 1 || got[0].Mode != yacycrawlcontract.CrawlRequestModeURL {
...
func TestExpanderExpandsSitelist(t *testing.T) {
	...
	req := yacycrawlcontract.CrawlRequest{
		...
		Mode:          yacycrawlcontract.CrawlRequestModeSitelist,
		ProfileHandle: "profile",
		ReferrerURL:   "ignored",
	}
...
func TestExpanderRejectsBadInputs(t *testing.T) {
	cases := [][]yacycrawlcontract.CrawlRequest{
		{{URL: "https://example.org/", Mode: "bad"}},
		{{URL: "://bad", Mode: yacycrawlcontract.CrawlRequestModeSitemap}},
		{{URL: "https://example.org/sitemap.xml", Mode: yacycrawlcontract.CrawlRequestModeSitemap}},
		{{URL: "https://example.org/list.txt", Mode: yacycrawlcontract.CrawlRequestModeSitelist}},
	}
...
func TestExpanderRejectsFetchFailures(t *testing.T) {
	source := seedSource{}
	for _, mode := range []yacycrawlcontract.CrawlRequestMode{
		yacycrawlcontract.CrawlRequestModeSitemap,
		yacycrawlcontract.CrawlRequestModeSitelist,
	} {
