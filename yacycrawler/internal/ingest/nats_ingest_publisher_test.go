...
func testIngestBatch(url string) ingest.IngestBatch {
...
func TestNATSIngestPublisherDelivers(t *testing.T) {
	...
	const count = 5
	want := make([]ingest.IngestBatch, 0, count)
	for i := range count {
...
func drainIngest(t *testing.T, js jetstream.JetStream, count int) []ingest.IngestBatch {
	t.Helper()
	...
	out := make([]ingest.IngestBatch, 0, count)
	for len(out) < count {
