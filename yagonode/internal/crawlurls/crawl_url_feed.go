...
func metadataItems(
	...
	referrers map[yagomodel.Hash]string,
) ([]crawlURLItem, error) {
...
func metadataItem(
	...
	if raw := row.Properties[yagomodel.URLMetaReferrer]; raw != "" {
		referrer = referrers[yagomodel.Hash(raw)]
	}

	title := decoded[yagomodel.URLMetaColDescription]

	return crawlURLItem{
		Title:       title,
		Link:        decoded[yagomodel.URLMetaURL],
		Referrer:    referrer,
		Description: title,
		Author:      decoded[yagomodel.URLMetaAuthor],
...
func metadataPubDate(row yagomodel.URIMetadataRow) string {
	modified := row.Properties[yagomodel.ColModDate]
