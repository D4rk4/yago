...
func TestHTMLNumberedPagesWindows(t *testing.T) {
	...
	first := htmlNumberedPages("/yacysearch.html", nil, resp, 1, 10)
	if len(first) != htmlPagerWindow || first[0].Number != 1 || !first[0].Current {
		t.Fatalf("first window = %+v", first)
	}

	tail := htmlNumberedPages("/yacysearch.html", nil, resp, 25, 10)
	...
	if got := htmlNumberedPages("/yacysearch.html", nil, single, 1, 10); got != nil {
