...
func TestRotateSeedNewsAttachesEncodedPublication(t *testing.T) {
	...
	wire, err := yagomodel.DecodeWireForm(ctx, attachment)
	if err != nil {
...
func mustRotatedID(t *testing.T, pool *Pool) string {
	t.Helper()
	wire, err := yagomodel.DecodeWireForm(context.Background(), pool.SeedNews(context.Background()))
	if err != nil {
		t.Fatalf("DecodeWireForm: %v", err)
	}
	record, err := ParseRecord(wire, fixedNow)
...
func TestAcceptNewsAttachmentReportsIntakeFailure(t *testing.T) {
	...
	pool.AcceptNewsAttachment(ctx, yagomodel.EncodeBase64WireForm(record.WireForm()))
