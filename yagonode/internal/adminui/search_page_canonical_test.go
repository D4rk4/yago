...
func TestCanonicalAdminSearchPage(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "past last", requested: 50, total: 45, page: 3, redirect: true},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			page, redirect := canonicalAdminSearchPage(
				test.requested,
				test.shown,
				test.total,
			)
			if page != test.page || redirect != test.redirect {
				t.Fatalf(
					"canonicalAdminSearchPage(%d, %d, %d) = (%d, %t), want (%d, %t)",
					test.requested,
					test.shown,
					test.total,
					page,
