...
func resultTopics(
	...
	for _, resource := range resources {
		...
		for _, word := range titleWords(title) {
			if _, isQueryTerm := excluded[yagomodel.WordHash(word)]; isQueryTerm {
				continue
			}
...
func termHashSet(terms []yagomodel.Hash) map[yagomodel.Hash]struct{} {
	if len(terms) == 0 {
		return nil
	}
	set := make(map[yagomodel.Hash]struct{}, len(terms))
