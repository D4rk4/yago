...
func (o failingObserver) PostingStored(*vault.Txn, yagomodel.Hash, yagomodel.Hash) error {
	return o.storeErr
}

func (o failingObserver) PostingPurged(*vault.Txn, yagomodel.Hash, yagomodel.Hash) error {
	return o.purgeErr
}
...
func TestIntakeReturnsBadPostingError(t *testing.T) {
	h := openHarness(t, 0, 100)
	_, err := h.rwi.Receiver.Receive(t.Context(), []yagomodel.RWIPosting{
		{WordHash: yagomodel.WordHash("w1"), Properties: map[string]string{}},
	})
	if err == nil {
...
func TestPurgePostingReportsAbsentPosting(t *testing.T) {
	...
	if err := storage.Update(t.Context(), func(tx *vault.Txn) error {
		var err error
		deleted, err = purger.PurgePosting(tx, yagomodel.WordHash("w1"), yagomodel.WordHash("u1"))
		if err != nil {
			return fmt.Errorf("purge posting: %w", err)
		}
...
func (failingPostingReceiver) Receive(context.Context, []yagomodel.RWIPosting) (Receipt, error) {
...
func (fakePostingReceiver) Receive(context.Context, []yagomodel.RWIPosting) (Receipt, error) {
	return Receipt{}, nil
}
...
func TestDecodeStoredPostingRejectsColumnError(t *testing.T) {
	...
	var mask [4]byte
	binary.LittleEndian.PutUint32(mask[:], 1<<uint(storedPostingColumnIndex[yagomodel.ColLanguage]))
	data.Write(mask[:])
	data.WriteByte(2)
