...
| --- | --- | --- | --- | --- |
| Peer liveness handshake | `/yacy/hello.html` | GET, POST | implemented | Returns caller IP, the locally observed caller type, own seed, and a bounded known seed list after rejecting self-pings and callers using this peer hash. After network authentication, a caller with a usable advertised endpoint is retained under the local callback result: successful callers can enter bounded senior membership, while failed callers remain persisted `junior` potential peers visible to Admin but excluded from reachable membership, hello and seed-list export, search candidates, and DHT targets. Callback work uses the upstream aggregate 6.5-second HTTP or 13-second HTTPS-first boundary, requires a nonnegative RWI-count response, replaces unspecified advertised literals with a usable trusted transport address, and persists the result under a separate bounded context. A later observation promotes or demotes the same caller. |
