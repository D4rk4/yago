...
Include values also narrow local Bleve URL candidates. The web provider receives
`site:` constraints for DNS names and IPv4 literals only when the complete
encoded expression fits its fixed query ceiling. IPv6 literals and oversized
expressions retain the base query. Exclusion wins over inclusion; bounded
overfetch and the final returned-host check remain authoritative and prevent a
disallowed high-ranked duplicate from consuming an allowed row's slot.

| Surface | Path | Methods | Status | Behavior |
| --- | --- | --- | --- | --- |
| Tavily-compatible search | `/search` | POST | partial | Defaults to five results. Every supported depth uses global local-plus-peer retrieval. `basic`, `fast`, and `ultra-fast` use `verify=false`; `advanced` uses `verify=ifexist`. The operator web policy remains authoritative, and `always` starts web retrieval in parallel for every depth. A zero-row response with incomplete sources returns retryable HTTP 503 with `Retry-After: 1` while the original caller context remains live. A child or source deadline is retryable while that caller remains live; inherited caller cancellation or deadline retains the infrastructure-error path. An honest complete miss remains HTTP 200 empty, and row-bearing partial responses remain HTTP 200. A default result contains `title`, `url`, `content`, `raw_content:null`, and a bounded score that decreases with served rank; it never exposes Yago provenance. `published_date` is news-only and normalized to `YYYY-MM-DD`; favicon and image fields appear only when requested, with URL strings or `{url,description}` entries according to the image-description flag. `max_results` accepts 0 through 20, advanced-only `chunks_per_source` accepts 1 through 3, include/exclude domain lists accept at most 300/150 entries, `country` is validated and general-only, and `safe_search` is rejected for fast depths. An accepted safe-search request forces the strongest documented filter on an eligible web engine, excludes unsupported engines before egress, and preserves the standard response shape. With click-capture exposure randomization disabled, equivalent root-portal and Tavily `advanced` requests preserve the same canonical URL order after deduplication. Equivalence requires the same query, parsed filters, false safe-search policy, result limit, and effective web consent. Requested usage is one request-local compatible unit for an executed basic, fast, or ultra-fast search and two for an executed advanced search; an unexecuted `max_results:0` request reports zero. |
...
`country` does not add a geographic boost, and `finance` has no dedicated
vertical. All supported search depths share global local-plus-peer retrieval.
They differ through verification and compatibility usage accounting:
`basic`, `fast`, and `ultra-fast` use `verify=false`; `advanced` uses
`verify=ifexist`. Exact portal ordering is promised only for equivalent
`advanced` requests.
