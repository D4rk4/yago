...
an explicit, acyclic graph of published Go interfaces over the `yagomodel` vocabulary; they
...
A small core carries shared vocabulary and mechanism with no feature logic: `yagomodel`,
`yagoproto`, the storage kernel `boltvault`, and the request-invariant guard `httpguard`.
