...
* Optional crawl seeding from web-fallback results SHALL run outside the search response path through two process-wide workers, admit at most 128 pending jobs, and give each admitted job a fresh background context that expires ten seconds after execution begins. A full pending queue SHALL warn and skip only the new optional job. URLs SHALL be normalized before admission, SHALL discard fragments, and SHALL reject credentials, missing hosts, non-HTTP schemes, and identities beyond the document URL limit. Every eligible surfaced URL SHALL receive a stored-document presence check lasting at most 50 milliseconds; an absent or lookup-indeterminate URL SHALL attempt a URL-idempotent durable automatic-discovery publication under the configured web-discovery profile, and at most one accepted order SHALL remain for that normalized URL. The discovery root SHALL be the warming fetch; the node SHALL NOT publish a second cache order for the same surfaced page.
