...
* Ordinary requested-hit retrieval over an analyzer-scoped disk index SHALL first request 32,769 unscored documents from the disjunction of every distinct positive lexical requirement across the same text, URL, language, dictionary, and fuzzy analyzers. Analyzer scope, exclusions, domain constraints, and optional expansion terms SHALL remain absent from that candidate query. Only a result that exhausts at 32,768 documents or fewer with no unseen tail MAY restrict the unchanged scoped query through a zero-boost document-identity conjunction. That final query SHALL remain authoritative for identities, ordering, BM25 scores, field scores, and totals. An explicit diagnostic explanation SHALL retain the exhaustive scoped query so its explanation tree is unchanged. A complete empty candidate set SHALL remain an honest empty answer. A sentinel, unknown tail, incomplete shard, cancellation, or operation failure SHALL NOT become a truncated success; an over-limit set SHALL retain exhaustive retrieval.
