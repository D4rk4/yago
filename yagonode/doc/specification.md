...
* Public search SHALL reject queries above 512 Unicode runes or 32 combined required and excluded parsed terms before retrieval. The interactive search pipeline SHALL enforce a 1.8-second end-to-end processing deadline and preserve completed local results when peer work reaches its deadline. YaCy `resource=local` and admin `scope=local` SHALL never use swarm or external web search. A Tavily-compatible `/search` request SHALL explicitly permit web search after its selected depth. Privacy-permitted web search SHALL default to an ordered miss cascade: local exact/morphology with any selected swarm search, local fuzzy recovery, then web search on a true miss. An operator MAY instead select parallel retrieval, which SHALL start web work alongside local and peer work on every eligible query and SHALL rank-fuse and deduplicate completed primary and web rankings. The complete exact local-plus-swarm and peer-evidence stage SHALL receive at most 600 milliseconds, fuzzy recovery 150 milliseconds, and the complete hedged web stage 950 milliseconds. Engine fetch-and-parse work SHALL admit at most eight attempts process-wide.
* Interactive retrieval SHALL cancel cooperative work before its hard response deadlines so completed partial results can survive. At most four interactive retrieval pipelines, four retained exact local-plus-peer stages, and four retained fuzzy local stages SHALL execute process-wide. Admission SHALL be nonblocking, and deadline or capacity exhaustion SHALL return an empty successful response carrying a stable partial failure instead of an HTTP error. A timed-out pipeline, exact stage, or fuzzy stage SHALL retain its admission until its underlying work actually exits. Exact-stage timeout or capacity exhaustion SHALL skip the redundant local fuzzy retry so an operator-permitted web branch keeps its response window.
...
* Click-derived ranking evidence SHALL be tied to a short-lived authenticated impression containing the exposed result identities, positions, and measured propensities; only bounded aggregates SHALL be retained. A comparable active revision SHALL be team-draft interleaved with the lexical baseline for online comparison; otherwise the node SHALL use adjacent FairPairs. Only statistically confident FairPairs winners SHALL become implicit relevance judgments, and team-draft credit SHALL NOT become qrels.
...
* The node MAY query an external DDGS provider according to the operator's privacy mode and start trigger. `disabled` SHALL not install or call the provider, `explicit` SHALL require request-level consent, and `enabled` SHALL permit it on every eligible global query without a request flag. The start trigger SHALL default to `miss`; `parallel` SHALL be an explicit operator choice and SHALL NOT weaken local-scope or privacy gates. The bounded operator-bearing submitted query SHALL be retained separately from the bare local/swarm terms and sent to the provider. Returned rows SHALL be filtered by every structured constraint verifiable from their URL, title, and snippet, including site, TLD, file type, in-URL text, and excluded terms. The provenance SHALL be recorded as `ddgs` and surfaced as a `web` badge on human search surfaces. External web search SHALL be disabled by default and SHALL route outbound queries through the egress guard. Cached provider responses SHALL be normalized to at most 20 rows per query with bounded title, URL, and snippet fields and SHALL share a 4 MiB/256-entry byte-aware cache.
