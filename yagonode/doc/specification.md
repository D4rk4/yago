...
* A Tavily-compatible search that produces zero rows, carries at least one incomplete source failure, and retains a live original caller context SHALL return HTTP 503 with `Retry-After: 1` and the existing detail-only error envelope. A child or source deadline while the original caller context remains live SHALL use that retryable availability response. Cancellation or a deadline inherited from the original caller SHALL retain the infrastructure-error path. A complete honest miss SHALL remain HTTP 200 with an empty result list, and a row-bearing partial response SHALL remain HTTP 200.
