...
* A failed relaxed local search SHALL preserve already validated strict results
  ...
  set SHALL skip scored analyzer retrieval. For an eligible miss-triggered web
  request with sufficient remaining time, execution-slot waiting SHALL reduce
  the primary stage budget while reserving the existing web and local recovery
  budgets plus 100 milliseconds for response assembly. First-seen-bounded
