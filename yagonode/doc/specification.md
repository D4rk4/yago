...
* Local retrieval SHALL require every query word to appear in a matching document (YaCy RWI-join parity), exempting words the query's candidate analyzers fold away entirely; recall-widening expansions (pseudo-relevance feedback, fuzzy recovery) SHALL only reorder or run on true misses and SHALL NOT surface documents lacking the required words.
* Local ranking SHALL build a bounded strict and relaxed lexical candidate union, preserve phrase and proximity evidence, and keep pseudo-relevance expansion bounded and drift-controlled.
...
* Learned ranking SHALL use a versioned fixed feature catalog, bounded model size and candidate windows, persistent active and rollback snapshots, and compare-and-swap activation. Until the training pool contains representative federated evidence, learned scoring SHALL reorder only locally stored candidates and SHALL preserve peer and web slots.
* Learned model promotion SHALL compare the proposal with both lexical ranking and the active incumbent on one frozen candidate pool, SHALL require at least 20 independent held-out query clusters and a non-negative 95% cluster-level paired-bootstrap lower bound, SHALL include chronological evidence when timestamps are present, and SHALL reject recall, discounted top-rank safety/spam exposure, named-slice, latency, peer-traffic, or timeout regressions.
* Click-derived ranking evidence SHALL be tied to a short-lived authenticated impression containing the exposed result identities, positions, and measured propensities; only bounded aggregates SHALL be retained.
...
* Peer-supplied results SHALL be capped at the requested row count, and reported remote totals SHALL count deduplicated rows in hand rather than peer-claimed join counts. When verification is enabled, first-page peer URLs SHALL be checked against fetched page text when reachable: matching pages receive query-biased snippets, confirmed no-match pages are demoted without removal, and unreachable pages remain eligible. `verify=false` SHALL skip the page-text verification pass.
