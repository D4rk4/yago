...
* Ordinary requested-hit retrieval over an analyzer-scoped disk index SHALL first request 32,769 unscored documents from the disjunction of every distinct positive lexical requirement across the same text, URL, language, dictionary, and fuzzy analyzers. Candidate clauses for one term MAY share the first analyzer whose exact token stream equals a preceding analyzer's stream; an unavailable analyzer and every CJK dictionary branch SHALL remain independent. Analyzer scope, exclusions, domain constraints, and optional expansion terms SHALL remain absent from that candidate query. Only a result that exhausts at 32,768 documents or fewer with no unseen tail MAY restrict the unchanged scoped query through a zero-boost document-identity conjunction. When every candidate carries one exact configured child identity and configured child names are nonempty and unique, each child SHALL resolve only the candidates it produced while the same Bleve alias retains scoring and merge authority. Missing, empty, duplicate, or unknown ownership SHALL use the complete alias-wide identity restriction. That final query SHALL remain authoritative for identities, ordering, BM25 scores, field scores, and totals. An explicit diagnostic explanation SHALL retain the exhaustive scoped query so its explanation tree is unchanged. A complete empty candidate set SHALL remain an honest empty answer. A sentinel, unknown tail, incomplete shard, cancellation, or operation failure SHALL NOT become a truncated success; an over-limit set SHALL retain exhaustive retrieval.
