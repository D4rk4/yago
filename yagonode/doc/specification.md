...
* Peer-supplied results SHALL be verified at ingest against the metadata the peer sent — a result none of whose title, snippet, or URL mentions a query term SHALL be dropped, one peer's answer SHALL be capped at the requested row count, and reported remote totals SHALL count verified rows in hand rather than peer-claimed join counts. The check SHALL follow the request's YaCy-compatible `verify` mode (default `ifexist`; `verify=false` trusts peers verbatim).
