...
| `YAGO_WEB_FALLBACK_TIMEOUT` | `10s` | Per-engine timeout ceiling. Interactive search additionally caps the complete hedged web stage at the shared 1.8-second response deadline, reserving 100ms for final assembly. Conditional searches reserve at least one second for web work when the caller deadline permits; execution-slot waiting reduces the primary budget. A smaller per-engine timeout still applies. |
...
| `YAGO_SEARCH_SESSION_TTL` | `5m` | How long a materialized public result window stays navigable, so a reader can page back into it. Accepts 30s to 1h. Admin key: `search.session.ttl`; applies live to sessions stored afterwards. The 500-row window size is a retrieval invariant, not a setting. |
| `YAGO_INGEST_QUALITY_GATE` | `true` | Apply the deterministic web-page quality gate before crawled HTML or plain text is stored and indexed. Admin key: `crawl.ingest.quality_gate`; takes effect after restart. Parsed document formats and unsegmented scripts follow the bounded exceptions described in the feature catalog. A refused page is not stored: it is counted by `crawl_ingest_low_quality_total`, logged at INFO with its source URL and the rule it violated, and reported back to the crawler, which records the URL as fetched rather than indexed. |
...
| `YAGO_SWARM_SEED_DEPTH` | `1` | Crawl depth for greedy-learning orders (0–8). |
| `YAGO_SWARM_SEED_MAX_PAGES` | `25` | Whole-run page cap for each greedy-learning crawl task. The global crawler run cap may reduce it further. |
...
dispatched with, so an automatic task stays an automatic task and keeps that
priority's page bounds. Legacy automatic-discovery profiles that predate the
whole-run field derive that limit from their stored positive per-host cap, so
