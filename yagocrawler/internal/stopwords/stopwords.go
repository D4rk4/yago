// Package stopwords mirrors the node's stopword list
// (yagonode/internal/stopwords) so the crawler's RWI postings and the node's
// query-side heuristics agree on which words carry no content: the crawler
// skips postings for these words, and the node never requires them of
// results. The entries derive from the Snowball per-language stopword lists,
// trimmed to the high-frequency words that dominate natural-language queries.
// Keep the two lists identical when editing either; the document-text (bleve)
// side is untouched, its analyzers filter on their own.
package stopwords
