...
func subsetFontPDF(t *testing.T, cmap string) []byte {
	...
	pdf.WriteString("5 0 obj\n<< /Type /Font /BaseFont /ABCDEF+X /ToUnicode 6 0 R >>\nendobj\n")
...
// TestParsePDFAppliesToUnicode pins the subset-font path end to end: the CMap
// resolves through the font object, code 0x02 reads back as "fi", and the
// unmapped ASCII around it falls back to the byte decoder.
...
// TestPDFCMapHelpers pins the token helpers: hex values bound their width and
// parse both cases, destinations decode UTF-16 with odd-length padding, the
// codespace defaults to one byte, and sections without an end marker drop.
func TestPDFCMapHelpers(t *testing.T) {
	...
	if got := pdfHexUTF16([]byte("004")); got != "@" {
		t.Fatalf("odd hex utf16 = %q", got)
	}
	if got := pdfHexUTF16([]byte("zz")); got != "" {
		t.Fatalf("junk hex utf16 = %q", got)
	}
...
// TestPDFMapString pins the mapped-string renderer: unmapped codes fall back
// per code, two-byte codes read big-endian, and an odd tail byte decodes
...
// TestPDFNameToken pins the /Name reader used for Tf font switches.
