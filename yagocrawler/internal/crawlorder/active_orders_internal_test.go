...
func TestActiveOrdersBoundsOneHundredSameLeaseReplays(t *testing.T) {
	...
	settlements := 0
	active.settle(provenance, delivery, true, func(CrawlOrderDelivery) {
		settlements++
	})
	if settlements != 1 {
...
func TestRequeuedLeaseIsNotRetainedAsCompleted(t *testing.T) {
	...
	active.settle(provenance, delivery, false, func(CrawlOrderDelivery) {})
	if claim := active.claim(provenance, delivery); claim != activeOrderStartsRun {
		t.Fatalf("requeued claim = %d, want start", claim)
	}
}

func TestShutdownCancelsQueuedRunWithoutProvenanceAndWaitsForNak(t *testing.T) {
	queue := boundedqueue.NewBoundedQueue[CrawlOrderDelivery](1)
	crawlFrontier := frontier.NewFrontier(1, nil)
	consumer := NewCrawlOrderConsumer(queue, crawlFrontier)
	profile := consumerProfile()
	naked := make(chan struct{})
	nakStarted := make(chan struct{})
	releaseNak := make(chan struct{})
	...
	select {
	case <-settled:
		t.Fatal("settlement wait returned before nak completed")
	...
	case <-settled:
	case <-time.After(time.Second):
		t.Fatal("settlement wait did not return after nak")
	}
	select {
	case <-naked:
	default:
		t.Fatal("queued shutdown order was not naked")
	}
...
func TestActiveOrdersSettlementFollowsNewLeaseClaim(t *testing.T) {
	...
	var settled []string
	active.settle(provenance, first, true, func(delivery CrawlOrderDelivery) {
	...
	active.settle(provenance, latest, true, func(CrawlOrderDelivery) {
...
func TestLeaseIdentityAndDuplicateCompletionStayBounded(t *testing.T) {
	...
	active.settle(nil, delivery, true, func(CrawlOrderDelivery) {})
	active.settle(alternateProvenance, delivery, true, func(CrawlOrderDelivery) {})
